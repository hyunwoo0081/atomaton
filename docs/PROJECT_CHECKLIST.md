...
- [x] **사용자 계정별** 로그 기록 및 보존 정책(최근 1,000건, 3일) 구현
  - _(추후 보존 기간을 설정할 수 있도록 확장성 있게 설계)_
- [x] 프론트엔드에 최신 로그를 제공하기 위한 API 엔드포인트 구현
...
- [x] `Vite`를 사용해 `React` + `TypeScript` 프로젝트 초기 설정
- [x] `react-router-dom`을 사용한 페이지 라우팅 설정
- [x] `fetch` API를 사용한 타입 세이프한 API 통신 클라이언트 설정

#### **3.2. 공용 UI 컴포넌트 (`packages/ui`)**

- [x] 버튼, 입력창, 카드 등 `React` + `TypeScript` 기반 공용 UI 컴포넌트 개발

#### **3.3. 사용자 화면**

- [x] 로그인 페이지 구현
- [x] 대시보드 메인 페이지 구현
- [x] **워크플로우 시각화 UI (`React Flow` 사용)**
  - [x] 노드 기반의 워크플로우 뷰어 구현
- [x] **워크플로우 설정 UI**
  - [x] Discord Action: 메시지 템플릿 입력 UI
  - [x] Notion Action: Database ID 입력 및 필드 매핑 UI
    - _(간단한 스크립트 매핑 기능은 추후 고려)_
- [x] 최신 실행 로그 뷰어 구현

#### **3.4. 개발자 전용 화면 (프론트엔드)**

- [x] **성능 모니터링 대시보드 UI**
  - [x] 워크플로우별 실행 시간, 리소스 사용량, 실행 빈도 등 성능 지표 시각화 (Mock Data 사용)
  - [x] 비효율적인 워크플로우 식별을 위한 분석 UI

---

### **Phase 4: 백엔드 확장**

#### **4.1. 개발자 전용 API (`apps/api`)**

- [x] **시스템 통계 API (`/admin/stats`)**
  - [x] 전체 사용자, 워크플로우, 실행 성공률 집계
  - [x] 워크플로우별 실패율 및 성능 지표 집계
...
- [x] E2E 테스트 (이메일 수신 → Notion 페이지 생성)
  - [x] 테스트 기획 및 테스트 시나리오 문서 작성
  - [x] 기획사와 비교하여 테스트 커버리지가 적절한지 확인
  - [x] playwright 를 이용한 테스트 작성
  - [x] API 통합 테스트를 통한 실제 데이터 흐름 검증
- [x] `apps/api`, `apps/web` 프로덕션 빌드 스크립트 작성 (github actions)
  - [x] Nx 등 모노레포 관리 툴이 필요한지 확인
- [x] 배포 가이드 작성 (`PM2`, `Nginx` 등)
- [x] 서버 환경변수 설정 가이드 작성 (`MASTER_KEY`, `DB_URL` 등)
