...
Atomaton은 GitHub Actions를 사용하여 지속적 배포(CD)를 수행합니다. 관련 워크플로우는 `.github/workflows/cd.yml`에 정의되어 있습니다.

### 6.1. GitHub Environments 설정

GitHub 저장소의 `Settings > Environments`에서 다음 두 개의 환경을 생성하세요:
1.  `production-frontend`
2.  `production-backend`

### 6.2. 필요한 Secrets 설정

GitHub 저장소에 아래 Secret들을 설정해야 합니다:

**Repository Secrets (전역 설정):**
- `SSH_HOST`: 서버의 IP 주소 또는 도메인.
- `SSH_USER`: SSH 접속 사용자명 (예: `ubuntu`).
- `SSH_PRIVATE_KEY`: 서버 접속을 위한 SSH 개인키.

**Environment Secrets (production-frontend 전용):**
- `PWD`: 프론트엔드 정적 파일이 배포될 서버의 절대 경로 (예: `/var/www/atomaton/apps/web/dist`).

**Environment Secrets (production-backend 전용):**
- `PWD`: 백엔드 프로젝트가 배포될 서버의 절대 경로 (예: `/var/www/atomaton`).

### 6.3. 워크플로우 동작 방식

- **프론트엔드**: CI 환경에서 빌드 후, 결과물(`dist`)만 `rsync`를 통해 서버의 Nginx 루트 경로로 전송합니다.
- **백엔드**: 소스 코드(불필요한 파일 제외)를 서버로 전송한 뒤, 서버에서 `pnpm install --prod`, `prisma migrate deploy`를 실행하고 `pm2`를 통해 프로세스를 재시작합니다.
