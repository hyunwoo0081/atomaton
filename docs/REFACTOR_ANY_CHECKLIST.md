...
이 문서는 프로젝트 내의 불분명한 타입을 단계별로 제거하고 구체화하기 위한 가이드라인입니다.

## 1. 개요

불분명한 타입은 TypeScript의 타입 시스템을 무력화시키고 런타임 에러의 원인이 됩니다. 모든 타입은 구체적인 인터페이스나 `unknown`으로 대체되어야 합니다.

## 2. 체크리스트

### 2.1. API Layer (apps/api)

- [x] `Request.userId` 확장 시 불분명한 타입 제거 -> `JwtPayload` 인터페이스 적용.
