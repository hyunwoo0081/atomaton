...
import type {
  AccountPayloadConfig,
  NaverImapAccountConfig,
  NotionAccountConfig,
  AccountResponse,
} from '../types/workflow'
...
export const AccountConnectionModal: React.FC<AccountConnectionModalProps> = ({
  isOpen,
  onClose,
}) => {
  const [type, setType] = useState<AccountResponse['type']>('NAVER_IMAP')
  const [name, setName] = useState('')
  const [config, setConfig] = useState<AccountPayloadConfig>(
    {} as AccountPayloadConfig
  )
        ...
        <form onSubmit={handleSubmit} className="p-8 flex-1 overflow-y-auto">
          <div className="mb-6">
            ...
            <select
              ...
              value={type}
              onChange={(e) =>
                setType(e.target.value as AccountResponse['type'])
              }
            >
              <option value="NAVER_IMAP" className="bg-[#0D0E12]">
                Naver Mail (IMAP)
              </option>
              <option value="NOTION" className="bg-[#0D0E12]">
                Notion
              </option>
            </select>
          </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input
                  label="IMAP Host"
                  value={
                    (config as NaverImapAccountConfig).host || 'imap.naver.com'
                  }
