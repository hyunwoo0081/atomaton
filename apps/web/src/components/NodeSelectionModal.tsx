...
export const NodeSelectionModal: React.FC<NodeSelectionModalProps> = ({
  ...
  return (
    <dialog
      ...
      className="fixed z-50 w-80 bg-[#0D0E12]/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/10 overflow-hidden border-0 p-0 outline-none backdrop:bg-transparent"
      style={{ top: position.y, left: position.x, margin: 0, inset: 'auto' }}
    >
      ...
      <div className="max-h-80 overflow-y-auto p-2 scrollbar-hide">
        {filteredNodes.map((node) => (
          ...
          >
            <div className="font-bold text-sm text-white group-hover:text-[#8A3FFC] transition-colors">
              {node.label}
            </div>
