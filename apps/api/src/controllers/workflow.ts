  ...
  try {
    const workflow = await prisma.workflow.create({
      data: {
        ...
        settings: { enableFailureAlert: false, failureWebhookUrl: '' } as unknown as Prisma.InputJsonValue,
      },
    });
...
export const getWorkflowById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!workflow) return res.status(440).json({ message: 'Workflow not found' });
...
export const updateWorkflow = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (nodes && edges) updateData.ui_config = { nodes, edges } as unknown as Prisma.InputJsonValue;
    if (globalSettings) updateData.settings = globalSettings as unknown as Prisma.InputJsonValue;
    ...
    if (nodes && edges) {
      await prisma.$transaction(async (tx) => {
        ...
        if (triggerNode) {
          const config = triggerNode.data.config as unknown as { accountId?: string };
          if (config.accountId) {
            await tx.trigger.create({
              data: {
                workflowId: id,
                type: triggerNode.type === 'trigger-webhook' ? 'WEBHOOK' : 'IMAP_POLLING',
                config: triggerNode.data.config as unknown as Prisma.InputJsonValue,
              }
            });
          }
        }
        ...
        for (let i = 0; i < actionNodes.length; i++) {
          ...
          await tx.action.create({
            data: {
              workflowId: id,
              type,
              config: { ...node.data.config, nodeId: node.id } as unknown as Prisma.InputJsonValue,
              order: i,
            }
          });
        }
...
export const testWorkflow = async (req: Request, res: Response) => {
  const { id } = req.params;
  const { nodes, edges, inputData } = req.body as { nodes: WorkflowNode[]; edges: WorkflowEdge[]; inputData: Record<string, unknown> };
  const userId = req.userId;
  if (!userId) return res.status(401).json({ message: 'Unauthorized' });
