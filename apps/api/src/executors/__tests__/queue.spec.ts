...
describe('Execution Queue', () => {
  ...
  it('should enqueue item after a delay', async () => {
    vi.useFakeTimers()
    ...
    enqueueWithDelay(mockContext('delayed'), 100)

    expect(processedIds).not.toContain('delayed')
    expect(getQueueSize()).toBe(0)

    vi.advanceTimersByTime(100)
    await vi.runAllTimersAsync()

    expect(processedIds).toContain('delayed')
    vi.useRealTimers()
  })
